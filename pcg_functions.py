...
def sound_to_spec(data, nperseg=944, noverlap=472, log_spectrogram = True):
...
def inference_all(model, now_location, recordings, data):
    ...
    '''
    classes = ['Absent','Present','Unknown'] #for murmur
    n_classes = 3
    classes2 = ['Abnormal','Normal'] #for outcome
    n_classes2 = 2
    max_length = 224000 #max_length = 56s
    ...
    for record in recordings:
        ...
        '''
        outputs = model['murmur_classifier'](pixel_values)
        outputs2 = model['outcome_classifier'](pixel_values)
        
        outputs = torch.cat((model['murmur_classifier'].vit(pixel_values)[0][0][0],meta_now.to(device)))
        outputs2 =torch.cat((model['outcome_classifier'].vit(pixel_values)[0][0][0],meta_now.to(device)))
        
        outputs = model['murmur_classifier'].classifier(outputs)
        outputs2 = model['outcome_classifier'].classifier(outputs2)
...
def inferenced_results(pred_arr, pred_arr2):
    ...
    output
    labels: final inferenced class per person
    probabilities: final inferenced probabilities per person
    '''    
    if 1 in pred_arr:
